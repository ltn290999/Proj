...
public class ShippingRateService {
    ...
    private static final int RATES_PER_PAGE = 10;
	...
	try {
	    ...
	    throw new ShippingRateNotFoundException("Cound not find shipping rate with ID: " + id);
	}
    }

    public void updateCODSupport(Integer id, boolean codeSupported) throws ShippingRateNotFoundException {
	Long count = shipRepo.countById(id);
	if (count == null || count == 0) {
	    throw new ShippingRateNotFoundException("Could not find shipping rate with ID: " + id);

	}
	shipRepo.updateCODSupport(id, codeSupported);
    }
